...
// Subnormal numbers and underflow results are flushed to zero
// Overflow results saturate to the largest representable finite value
...
module mkFloatAdd(FloatTwoOp);
	...
	rule procIn;
		...
		Bit#(1) signa = bina[31];
		Bit#(1) signb = binb[31];
		Bit#(18) fraca = (expa == 0)?0:(zeroExtend(bina[22:6])|(1<<17));
		Bit#(18) fracb = (expb == 0)?0:(zeroExtend(binb[22:6])|(1<<17));
	...
	rule normalize4;
		...
		if ( newfrac == 0 || newexp == 0 ) begin
			outQ.enq(0);
		end else if ( newexp == 8'hff ) begin
			outQ.enq({newsign,8'hfe,17'h1ffff,6'b0});
		end else begin
			outQ.enq({newsign,newexp,newfrace,6'b0});
		end
	endrule
