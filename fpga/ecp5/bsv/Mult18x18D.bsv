...
typedef 3 Mult18x18DLatency;
typedef 4 Mult18x18DQueueDepth;
...
module mkMult18x18DBSIM(Mult18x18DBSIMIfc);
	FIFO#(Bit#(18)) aQ <- mkSizedFIFO(valueOf(Mult18x18DQueueDepth));
	FIFO#(Bit#(18)) bQ <- mkSizedFIFO(valueOf(Mult18x18DQueueDepth));
...
`endif
	FIFOF#(Bit#(36)) outQ <- mkSizedFIFOF(valueOf(Mult18x18DQueueDepth));
	Wire#(Bit#(1)) validWire <- mkDWire(0);
	Reg#(Bit#(Mult18x18DLatency)) validMap <- mkReg(0);
...
`endif
		...
		if ( validMap[valueOf(Mult18x18DLatency) - 1] == 1 ) begin
`ifdef BSIM
...
`endif
	...
	method Action put(Bit#(18) a, Bit#(18) b)
		if ( dataInFlightUp - dataInFlightDn < fromInteger(valueOf(Mult18x18DQueueDepth)) );
		wireA <= a;
		wireB <= b;
		validWire <= 1;
