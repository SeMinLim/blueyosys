...
void compress_1d(float original[4], BitBuffer* output, int bit_budget) {
	int exp_max = -EXP_MAX;
	for ( int i = 0; i < 4; i++ ) {
		if ( original[i] != 0 ) {
			int exp = 0;
...
void* swmain(void* param) {
	...
	float* decomp_outputs = (float*)malloc(sizeof(float)*input_cnt*output_dim);
	uint8_t* comp_outputs = (uint8_t*)malloc(sizeof(uint8_t)*input_cnt*output_dim);
	...
	for ( size_t i = 0; i < input_dim; i ++ ) {
		for ( size_t j = 0; j < output_dim; j += 4 ) {
			...
			compress_1d(original, output, bit_budget);
	...
	nn_fc(comp_weights, comp_bias, comp_inputs, comp_add, input_cnt, input_dim, output_dim, comp_outputs);
	//Decompressing output values 
	uint8_t comp[4];
	...
	for ( size_t i = 0; i < input_cnt; i ++ ) {
		for ( size_t j = 0; j < output_dim; j ++ ) {
			...
			for ( size_t k = 0; k < 4; k ++ ) {
				comp[k] = comp_outputs[i*output_dim+j];
			}
			decompress_1d(comp, compressed, decompressed, bit_budget);
			for ( size_t l = 0; l < 4; l ++ ) {
				decomp_outputs[i*output_dim+j] = decompressed[l];
			}
			delete compressed;
		}
	}
	//Reordering output values
	size_t in_idx = 0;
	size_t out_idx = 0;
	size_t in_cnt = 0;
	size_t out_cnt = 0;
	for ( size_t i = 0; i < input_cnt; i ++ ) {
		for ( size_t j = 0; j < output_dim; j ++ ) {
			outputs[(in_idx*output_dim)+out_idx] = decomp_outputs[(i*output_dim)+j];
